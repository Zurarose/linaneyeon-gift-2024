...
const Items = [
  {name: 'килукити', image: {cape: BoxCapeImage1.src, body: BoxBodyImage1.src}, description: 'килукити', color: '#f78db4'},
  ...
  //box missing
  {name: 'Dr1ve_Kn1ght', image: {cape: BoxCapeImage13.src, body: BoxBodyImage13.src}, description: 'Поздравляю с днем рождения Лин оставайся такой же милой и уютным владельцем Таверны Персики ❤️', color: '#4B0082'},
  {name: 'Арсван', image: {cape: BoxCapeImage13.src, body: BoxBodyImage13.src}, description: 'Лина, поздравляю тебя с днём рождения! Ты лучик солнца для многих людей, который зажигает огонь в их сердцах. Хочу пожелать всего самого наилучшего, быть здоровой, оставаться такой же доброй, умной и красивой девушкой, добиваться своих целей, собирать вокруг себя только приятных тебе людей и всегда быть счастливой. Ну и пожелаю тебе взять мастера в лиге, всегда выигрывать 50 на 50 в гачах и интересных детективных дорам :3', color: '#ff2f2f'},
  {name: 'unicorn', image: {cape: BoxCapeImage13.src, body: BoxBodyImage13.src}, description: 'С днём рождения, Лина! Незаметно прошёл годик, а ты не просто всё также хороша, но и стала лучше — случай, когда перетекающие 19 в 20 лет действительно красят. Надеюсь, в новом году ты порешаешь часть вопросов со здоровьем, ибо болеть — отвратно. Также прокачаешь навыки коммуникации, чтобы качество общения со зрителями и чатом выросло (что бы это ни значило). И, конечно, будешь находить столько времени на хобби, сколько хочется.', color: '#8b00ff'},
  {name: 'Кошмарик', image: {cape: BoxCapeImage13.src, body: BoxBodyImage13.src}, description: 'Здравствуй Линён-тан. Хочу поздравить тебя с этим знаменательным днём. С Днём твоего варенья, ты стала большой девотькой. Пусть жизнь твоя будет сладка как персиковое варенье, только пальчики не измажь. Желаю чтоб твоя ослепительно-яркая улыбка светила еще ярче чем когда-либо еще. Большинство невзгод не тревожили твое сердечко и здоровье было крепким =^_^=', color: '#696969'},
  ...
  return (
      <React.Fragment>
          ...
          <div className={cn("relative justify-between items-center", !!modalData && 'pointer-events-none touch-none')}>
            <div className={cn((opennedCount !== Items?.length) && 'cursor-not-allowed')}>
            ...
            {/* <div id={Items?.[0]?.name} className="absolute bottom-[-180px] left-[-260px] z-30" onClick={openModal(Items[0])}>
              <SmallBox {...Items[0]} />
            </div>
            <div id={Items?.[0]?.name} className="absolute bottom-[-120px] left-[-390px] z-20" onClick={openModal(Items[0])}>
              <SmallBox {...Items[0]} />
            </div> */}
          </div>
      </React.Fragment>
...
const PresentBoxMain = ({className}: {className?: string}) => {
  ...
  return (
    <>
      ...
      <div onClick={onOpen} className={cn("drop-shadow-2xl relative mt-32 flex flex-col justify-center items-center group", !isOpened ? "shake" : 'bounce', className)}>
